...
Infrastructure de **développement local partagée** pour plusieurs projets.
...
comme **sous-module git**. Il applique un principe simple aux worktrees : le serveur PostgreSQL local
est partagé, tandis qu'une base de données isole chaque consommateur.
...
| --------- | --------------------- | ----------------------- | ------------------------------------------------------ |
| Postgres  | `postgres-dev-shared` | `5433`                  | Base `postgres` par défaut + une base distincte par application |
...
un **Quick Link** ciblant directement leur base. Prisma Studio est **spécifique au schéma** d'une
app, donc il reste côté stack hôte (par exemple via `make prisma-studio`).
...
par les apps. **Chaque app crée sa propre base** via son propre processus de setup. Ce dépôt ne crée
...
2. Déléguer les cibles Make du stack hôte vers le sous-module :
   ...
   ```
   La base applicative est créée par le processus de setup du stack hôte, pas ici.
3. Pointer le `DATABASE_URL` du stack vers la base partagée :
   ```
   DATABASE_URL=postgres://postgres:postgres@localhost:5433/<database>
   ```
